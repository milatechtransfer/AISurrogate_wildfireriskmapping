...
# full national-grid float32 array in memory per target (sequentially, one at a time).
# For a Canada-wide raster at fine resolution this can be tens of GB -- adjust --mem
# above to comfortably fit your reference raster's (height * width * 4 bytes) footprint.
# You can check your reference raster's size with:
