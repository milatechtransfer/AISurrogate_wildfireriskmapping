#!/bin/bash
# Submit three seeded q3 evaluations, per-seed plots, and final mean/std
# aggregation for the paper counterfactual scenarios.
#
# Usage:
#   bash run_files/counterfactual/submit_multiseed_counterfactuals.sh [--dry-run] [name ...]
#
...
for entry in "${selected[@]}"; do
    ...
    if [[ ${dry_run} -eq 1 ]]; then
        echo "sbatch --array=0-2 --job-name=cf_${name}_eval --export=${exports} ${eval_script}"
        echo "sbatch --array=0-2 --dependency=afterok:<eval> --job-name=cf_${name}_plots --export=${exports} ${plot_script}"
        echo "sbatch --dependency=afterok:<eval> --job-name=cf_${name}_agg --export=${exports} ${aggregate_script}"
        printf '%-16s %-12s %-12s %-12s\n' "${name}" "DRYRUN" "DRYRUN" "DRYRUN"
        continue
    fi

    eval_job=$(sbatch --parsable --array=0-2 --job-name="cf_${name}_eval" --export="${exports}" "${eval_script}")
    eval_job="${eval_job%%;*}"
    plot_job=$(
        sbatch --parsable \
            --array=0-2 \
