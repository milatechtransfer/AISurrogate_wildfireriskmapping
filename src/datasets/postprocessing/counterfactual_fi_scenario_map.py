"""Generic FI response figures for a single counterfactual scenario."""
...
from src.datasets.postprocessing.counterfactual_fwi_map import GT_RELATIVE_PATH
...
def load_fi_response(
    ...
    *,
    ...
    if scenario_fi.shape != baseline.shape:
        raise ValueError(f"Scenario FI shape {scenario_fi.shape} does not match baseline grid {baseline.shape}.")

    extent = read_prediction_extent(baseline_path)
    with rasterio.open(baseline_path) as src:
        reference_profile = src.profile.copy()

    finite = ~np.ma.getmaskarray(baseline) & ~np.ma.getmaskarray(scenario_fi)
    ...
    return ground_truth, baseline, scenario_fi, delta, extent, reference_profile
...
def plot_fi_response_maps(
    ...
    *,
    ...
    pooled = np.concatenate([finite_values(ground_truth), finite_values(baseline), finite_values(scenario_fi)])
    fi_norm = Normalize(vmin=0.0, vmax=float(np.percentile(pooled, 99.0)))
    ...
    panels = [
        (ground_truth, "Ground truth FI (BurnP3+)", "inferno", fi_norm, "Fire intensity (kW/m)"),
        (baseline, "Baseline FI (model)", "inferno", fi_norm, "Fire intensity (kW/m)"),
        (scenario_fi, f"Scenario FI \u2014 {scenario_label}", "inferno", fi_norm, "Fire intensity (kW/m)"),
        (delta, "\u0394FI (scenario \u2212 baseline)", "RdBu_r", delta_norm, "\u0394 Fire intensity (kW/m)"),
    ]
...
def plot_fi_patch_zoom(
    ...
    *,
    ...
    for row, (center_row, center_col) in enumerate(centers):
        ...
        panels = [
            (ground_truth[r0:r1, c0:c1], "Ground truth FI (BurnP3+)", "inferno", fi_norm, "Fire intensity (kW/m)"),
            (baseline[r0:r1, c0:c1], "Baseline FI", "inferno", fi_norm, "Fire intensity (kW/m)"),
            (scenario_fi[r0:r1, c0:c1], f"Scenario FI \u2014 {scenario_label}", "inferno", fi_norm, "Fire intensity (kW/m)"),
            (delta[r0:r1, c0:c1], "\u0394FI", "RdBu_r", delta_norm, "\u0394 Fire intensity (kW/m)"),
        ]
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--out_dir", type=Path, default=None, help="Defaults to experiment_dir/figures/<scenario>_fi.")
...
def main() -> None:
    ...
    out_dir = args.out_dir if args.out_dir is not None else args.experiment_dir / "figures" / f"{args.scenario}_fi"
    ...
    ground_truth, baseline, scenario_fi, delta, extent, reference_profile = load_fi_response(
        prediction_dirs,
        args.hex_id,
        raw_data_dir,
    ...
    plot_fi_response_maps(
        ground_truth,
        baseline,
        scenario_fi,
        delta,
        extent,
        out_path=out_dir / f"{args.scenario}_fi_response_maps.png",
        scenario_label=label,
        suptitle=f"FI response \u2014 {label} (hex 16)",
    ...
    plot_fi_patch_zoom(
        ...
        scenario_fi,
        delta,
        out_path=out_dir / f"{args.scenario}_fi_patch_zoom.png",
        scenario_label=label,
    ...
    plot_delta_histogram(
        delta,
        out_path=out_dir / f"{args.scenario}_fi_delta_distribution.png",
        xlabel="\u0394FI per pixel (scenario \u2212 baseline, kW/m)",
        title=f"Per-pixel FI change \u2014 {label} (hex 16)",
        color=DELTA_COLOR,
    )

    values = finite_values(delta)
    print(f"{args.scenario}: n={values.size} mean_dFI={float(np.mean(values)):+.1f} median={float(np.median(values)):+.1f}")
